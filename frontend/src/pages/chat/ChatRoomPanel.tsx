...
export function ChatRoomPanel({
  ...
  return (
    <Panel open={open} title={`和${peer.name ?? 'TA'}一起玩`} onClose={onClose}>
        ...
        <div className="mb-4 flex items-center gap-3 rounded-lg border border-danger/30 bg-danger/10 px-3 py-2 text-sm text-danger">
          ...
          {!data && (
            <button
              type="button"
              onClick={() => void load()}
              className="shrink-0 rounded-lg border border-danger/40 px-2.5 py-0.5 text-xs transition-colors hover:bg-danger/10"
            >
              重试
            </button>
          )}
      ...
      {!data && !error && <p className="py-10 text-center text-sm text-ink-faint">正在看看有什么…</p>}

      {data && (
        <div className="space-y-6">
          <section>
            <SectionHead title="已经开着" count={data.open.length} />
            {data.open.length === 0 ? (
          ...
          <section>
            <SectionHead title="可以开一个" count={data.openable.length} />
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-line py-2 text-xs text-ink-soft transition-colors hover:bg-sunken hover:text-ink"
          >
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-line bg-raised px-3 py-2.5">
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate font-mono text-xs text-ink">{session.applicationId}</div>
        <div className="mt-0.5 tnum text-[11px] text-ink-faint">
      ...
      <button
        type="button"
        onClick={onEnter}
        className="shrink-0 rounded-lg border border-line px-3 py-1 text-xs text-ink-soft transition-colors hover:bg-sunken hover:text-ink"
      >
        进入
      </button>
    </div>
