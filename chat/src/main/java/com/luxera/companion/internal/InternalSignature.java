 ...
 * "这条请求确实是我发的, 且没被改过"。
 *
 * <p>模式与本仓 R14 {@code application/.../remote/RemoteSignature.java}（远端应用签名）
 * 完全一致: 签 {@code <timestamp>.<body>} 而不是只签 body —— 只签 body 时, 拿到历史
 * 请求的人可以原样重放到明天。把时间戳纳入签名, 接收方就能用时间窗把重放挡掉。
 *
 * <p>与仓 2 {@code agentserver/internal/InternalSignature} 各持一份（contract 是纯
 * 类型模块、不依赖 Spring, 这类工具不便下放）, 两份必须语义逐字节一致 —— 任何一边
 * 改了签名材料都要同步。
 */
